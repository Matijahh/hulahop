...
const EditAssociateStore = () => {
  ...
  const getStoreData = async () => {
    ...
    const response = await commonGetQuery(`/store_layout_details/${params.id}`);
  ...
  return (
    <CommonWhiteBackground>
      <Helmet>
        <title>{t("Edit Associate Store - Admin")}</title>
      </Helmet>
      <FlexBox className="mb-4" isWrap>
        <div className="main-title title-wrapper">
          {t("Edit Associate Store")}
        </div>
        {formik.values.id && (
          <ButtonComponent
            variant="contained"
            text={t("View Associate Store")}
            onClick={() =>
...
export default connect(mapStateToProps, mapDispatchToProps)(EditAssociateStore);
